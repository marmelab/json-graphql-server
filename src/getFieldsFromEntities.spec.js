import { getValuesFromEntities } from './getFieldsFromEntities';

describe('getValuesFromEntities', () => {
    test('does not take empty values into account', () => {
        expect(
            getValuesFromEntities([{ foo: null }, {}, { foo: 'bar' }]),
        ).toEqual({
            foo: ['bar'],
        });
    });
    ...
    test('does not ignore duplicate values', () => {
        expect(
            getValuesFromEntities([{ foo: 'bar' }, { foo: 'bar' }]),
        ).toEqual({
            foo: ['bar', 'bar'],
        });
    });
