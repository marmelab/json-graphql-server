...
MockHttpRequest.prototype = {
        ...
        if (
            ...
            if (
                ...
                (xmlDoc &&
                    ...
                    xmlDoc.documentElement.firstChild.firstChild.nodeName ==
                        'parsererror')
            ) {
                xmlDoc = null;
            }
    ...
    parseUri: function(str) {
        var pattern = /^(?:([^:\/?#]+):)?(?:\/\/((?:(([^:@]*)(?::([^:@]*))?)?@)?([^:\/?#]*)(?::(\d*))?))?((((?:[^?#\/]*\/)*)([^?#]*))(?:\?([^#]*))?(?:#(.*))?)/; // eslint-disable-line no-useless-escape
        var key = [
            'source',
            'protocol',
            'authority',
...
MockHttpServer.prototype = {
    ...
    /**
     * Receive the XMLHttpRequest as sole argument
     */
    handle: function() {
        // Instances should override this.
    },
